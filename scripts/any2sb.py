...
def get_patch_dict(proj_dir):
    ...
    # 读取象码扩展词库中的词语
    sbfm_extended_path = proj_dir / 'sbxm.extended.dict.yaml'
    with open(sbfm_extended_path, 'r', encoding='utf-8') as s:
        print(f'☑️  已加载象码扩展数据 » {sbfm_extended_path}')   
...
def any2sb(proj_dir):
    ...
    src_dir = (proj_dir / '..' / 'patches' / 'cn_dicts_frost').resolve()
...
''')
        ...
        for word in list(dict.fromkeys(word_weight_dict.keys())):
        # for word in list(dict.fromkeys(sorted(words_total))):
            ...
            for code in code_list:
                o.write(f'{word}\t{code}\t{word_weight_dict[word]}\n')
